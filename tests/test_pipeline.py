...
def setup_data_paths(tmp_path):
    ...
    test_params["data"]["raw_dataset_path"] = str(
        data_dir / os.path.basename(params["data"]["raw_dataset_path"])
    )
    test_params["data"]["clean_dataset_path"] = str(
        data_dir / os.path.basename(params["data"]["clean_dataset_path"])
    )
    test_params["data"]["preprocessed_features_path"] = str(
        data_dir / os.path.basename(params["data"]["preprocessed_features_path"])
    )
    test_params["model"]["path"] = str(
        tmp_path / os.path.basename(params["model"]["path"])
    )
    test_params["reports"]["dir"] = str(reports_dir)
    test_params["reports"]["classification_report_path"] = str(
        reports_dir / os.path.basename(params["reports"]["classification_report_path"])
    )
    test_params["reports"]["confusion_matrix_path"] = str(
        reports_dir / os.path.basename(params["reports"]["confusion_matrix_path"])
    )
    ...
    pd.read_csv(sample_data_path).to_csv(
        test_params["data"]["raw_dataset_path"], index=False
    )
...
def test_pipeline_steps(setup_data_paths):
    ...
    ), "Preprocessed features should be created"
